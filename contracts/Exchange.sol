...
contract Exchange is owned {
    ...
    function buyToken(string symbolName, uint priceInWei, uint amount) public {
        ...
        if (tokens[tokenNameIndex].amountSellPrices == 0 || tokens[tokenNameIndex].curSellPrice > priceInWei) {
            ...
            //deduct the amount of ether from our balance
            ...
            uint offers_key;
            while (whilePrice <= priceInWei && amountNecessary > 0) { // start with the smallest sell price.
                offers_key = tokens[tokenNameIndex].sellBook[whilePrice].offers_key;
                while (offers_key <= tokens[tokenNameIndex].sellBook[whilePrice].offers_length && amountNecessary > 0) { // first order (FIFO)
                    uint volumeAtPriceFromAddress = tokens[tokenNameIndex].sellBook[whilePrice].offers[offers_key].amount;

                    // one person offers not enough volume to fulfill the market order - we use it up completely and move on to the next person who offers the symbolName
                    // else: we make use of parts of what a person is offering - lower his amount, fulfill out order.
                    if (volumeAtPriceFromAddress <= amountNecessary) {
                        ...
                        // overflow check
                        // offer is <= the volume that we ask for, it is all used
                        ...
                        // overflow check
                        // offers is > we ask for; reduce his balance, add tokens to us, add ether to him.
                ...
                // set the curSellPrice again., when volume is exhausted @ a lowest price, curSellPrice is set there
